...
bgImage.src = 'imagesJsIndividual/backgroundJsIndi.jpg';

//Silly splash image
let iamMeImg = new Image();
iamMeImg.src = 'imagesJsIndividual/iamMe.png';

// Player sprites
let playerIdleImg = new Image();
playerIdleImg.src = 'imagesJsIndividual/thePlayer.png';
...
frameFiles.forEach(function (file) {
    const img = new Image();
    img.src = 'imagesJsIndividual/' + file;
    playerRunFrames.push(img);
});

// Object textures
let clockImg = new Image();
clockImg.src = 'imagesJsIndividual/clock.png';
let starImg = new Image();
starImg.src = 'imagesJsIndividual/star.png';
let emptyStarImg = new Image();
emptyStarImg.src = 'imagesJsIndividual/emptyStar.png';

let platformImg = new Image();
platformImg.src = 'imagesJsIndividual/platform.png';
let floorImg = new Image();
floorImg.src = 'imagesJsIndividual/floor.png';
let pencilImg = new Image();
pencilImg.src = 'imagesJsIndividual/pencil.png';
let crateImg = new Image();
crateImg.src = 'imagesJsIndividual/crate.png';
let doorImg = new Image();
doorImg.src = 'imagesJsIndividual/door.png';

// ==================== Audio ====================
let bgMusic = new Audio('soundsJsIndividual/bkgMusic.mp3');
bgMusic.loop = true;
bgMusic.volume = 0.5;
bgMusic.preload = 'auto';

let splashMusic = new Audio('soundsJsIndividual/splashMusic.mp3');
...
class Game {
    ...
    nextLevel() { if (this.levelIndex < this.levels.length - 1) { this.levelIndex++; this.loadLevel(this.levelIndex); } }

    /** Goes back to the previous level if not on the first level. */
    prevLevel() { if (this.levelIndex > 0) { this.levelIndex--; this.loadLevel(this.levelIndex); } }
...
// ==================== Initialization ====================
function init() {
    // Get references to all DOM elements for later manipulation

    // Splash screen elements
    splashCanvas = document.getElementById('splashCanvas');           // canvas for splash animation
    splashCtx = splashCanvas.getContext('2d');                        // drawing context for splash

    // Main game canvas
    canvas = document.getElementById('gameCanvas');                   // canvas where game is rendered
    ctx = canvas.getContext('2d');                                    // 2D drawing context for game

    // Containers
    splashDiv = document.getElementById('splash');                    // splash screen container
    gameContainer = document.getElementById('gameContainer');         // main game container (hidden initially)

    // UI elements for level info and messages
    historyDiv = document.getElementById('history');                  // container for best times history
    levelStatus = document.getElementById('levelStatus');             // shows current level (e.g., "level 1 / 3")
    levelNameEl = document.querySelector('header h1');                // level name displayed in header
    feedbackDiv = document.getElementById('feedback');                // temporary messages (level complete, etc.)
    gameoverDiv = document.getElementById('gameover');                // game over / history screen

    // Buttons
    startBtn = document.getElementById('startBtn');                   // start game from splash screen
    helpBtn = document.getElementById('helpBtn');                     // opens help panel
    helpPanel = document.getElementById('helpPanel');                 // the help panel itself
    replayBtn = document.getElementById('replayBtn');                 // play again after game over
    restartBtn = document.getElementById('restartBtn');               // restart current level
    prevLevelBtn = document.getElementById('prevLevelBtn');           // go to previous level
    nextLevelBtn = document.getElementById('nextLevelBtn');           // go to next level
    jumpBtn = document.getElementById('jumpBtn');                     // jump button (touch/mouse)
    zoomOutBtn = document.getElementById('zoomOutBtn');               // zoom out when object selected
    zoomInBtn = document.getElementById('zoomInBtn');                 // zoom in when object selected
    rotateBtn = document.getElementById('rotateBtn');                 // rotate selected object
    selectionControls = document.getElementById('selectionControls'); // container for zoom/rotate buttons
    pauseOverlay = document.getElementById('pauseOverlay');           // overlay shown when help is open
    moveLeftBtn = document.getElementById('moveLeftBtn');             // left movement button
    moveRightBtn = document.getElementById('moveRightBtn');           // right movement button

    setupEventListeners();
    animateSplash();
    // After 2 seconds, change splash message and show start button
    setTimeout(function () {
        document.getElementById('splashMsg').textContent = 'READY?';
        startBtn.classList.remove('hidden');
    }, 2000);

    // Attempt to play splash music (may be blocked by autoplay policies)
...
function setupEventListeners() {
    ...
    restartBtn.addEventListener('click', function () {
        if (game) {
            ...
            // Clear any pending pendulum feedback when restarting
            if (pendulumFeedbackTimeout) {
                clearTimeout(pendulumFeedbackTimeout);
                pendulumFeedbackTimeout = null;
                feedbackDiv.classList.add('hidden');
...
function updateLevelStars(levelIndex) {
    ...
    for (let i = 1; i <= 3; i++) {
        ...
        if (starEl) {
            const starSrc = i <= stars ? 'imagesJsIndividual/star.png' : 'imagesJsIndividual/emptyStar.png';
